...
**❓ What's wrong? What's the difference between your local environment and image environment? 💬 Discuss with your buddy.**
...
- 💻 Update your `Dockerfile` base image with either `tensorflow/tensorflow:2.10.0` (if you are on Intel processor only)
...
👏 Everything runs fine on your local machine. Great. We will now deploy your image on servers which are going to run these container online for you.

However, note that these servers (Google Cloud Run servers) will be running on **AMD/Intel x86 processors**, not ARM/M1, as most cloud provider still run on Intel.
